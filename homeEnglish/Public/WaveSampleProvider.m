...
- (void) informDelegateOfFinish
{
	if(delegate != nil) {
		if([delegate respondsToSelector:@selector(sampleProcessed:)]) {
			[delegate sampleProcessed:self];//wenyuan
		}
	}
}
...
{
	[self processSample];	
	[self performSelectorOnMainThread:@selector(informDelegateOfFinish) withObject:nil waitUntilDone:NO];//wenyuan
}
...
- (void) processSample
{
	...
	while (!extAFReachedEOF) {
		...
		[self calculateSampleFromArray:audio lenght:k];//wemyuan
		packetReads += k;
	}
...
- (void) normalizeSample
{
	...
	for(NSNumber *num in sampleData) {
		...
		[normalizedData addObject:nval];//wenyuan
	}	
	[sampleData release];
	sampleData = nil;
}
...
- (void) calculateSampleFromArray:(float**)audio lenght:(int)length
{
	double maxValues[extAFNumChannels];
	for(int i = 0; i < extAFNumChannels;i++) {
		maxValues[i] = 0.0; 
	}
...
- (float *)dataForResolution:(int)pixelWide lenght:(int *)length
{
	...
	int k = 0;
	    NSLog(@"count=%d",normalizedData.count);
...
//		float valMax = 0;
		...
		for(int j = 0; j < rangeLength; j++) {
			...
			if(idx < normalizedData.count) {
				NSNumber *nVal = [normalizedData objectAtIndex:idx];//wenyuan
				float val = nVal.floatValue;
                valueAverage+=val;
			//	if(valMax < val) valMax = val;
			}
		}
        valueAverage/=rangeLength;
		ret[k] = valueAverage;//wenyuan
		k++;
	}
	*length = k;
	return ret;
