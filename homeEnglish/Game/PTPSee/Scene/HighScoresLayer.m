...
-(id) init{
	...
	CCLabelTTF *timeLabel = [CCLabelTTF labelWithString:@"Time Records" fontName:@"Marker Felt" fontSize:21];
	...
	for (int i=0; i<[timeRecords count]; i++) {
		...
		CCLabelTTF *labelName = [CCLabelTTF labelWithString:each.name fontName:@"Marker Felt" fontSize:15];
		labelName.position = ccp(70, 250-30*i);
		[self addChild:labelName];
		
		CCLabelTTF *labelValue = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%d", each.time] fontName:@"Marker Felt" fontSize:15];
		labelValue.position = ccp(170, 250-30*i);
		[self addChild:labelValue];
	}
	
	CCLabelTTF *scoreLabel = [CCLabelTTF labelWithString:@"Score Records" fontName:@"Marker Felt" fontSize:21];
	...
	for (int i=0; i<[scoreRecords count]; i++) {
		...
		CCLabelTTF *labelValue = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%d", each.score] fontName:@"Marker Felt" fontSize:15];
