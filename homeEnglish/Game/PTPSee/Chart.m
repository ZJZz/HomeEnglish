...
-(void) dismissTile: (Tile *) tile{
	...
	sprite.scaleX = kDefaultScaleX*scaleXY*scaleSF;
	sprite.scaleY = kDefaultScaleY*scaleXY*scaleSF;
	
	//物品图片消失的位置
	CCSequence *someAction = [CCSequence actions:
							  [CCEaseBackIn actionWithAction:
							  [CCSpawn actions:
							   [CCMoveTo actionWithDuration: 0.7F position:ccp(55,560)],
...
//    CCLOG(@"chart.m fillContent count01:%d",count);
    ...
    CCLOG(@"chart.m fillContent count02:%d",count);
	...
	for (int i=0; i < count; i++, i++) {
		int number = i;
//		if (i>=level.factor) {
//        CCLOG(@"[PlayLayer wordCountF]:%d",[PlayLayer wordCountF]);
//        CCLOG(@"[level.factor]:%d",level.factor);
        if (i>=[PlayLayer wordCountF]) {
			number = arc4random()%[PlayLayer wordCountF];
		}
		tempResult[i] = number;
...
//                    CCLOG(@"00 index :%d",index);
					NSNumber *number = [indexArray objectAtIndex: index];
					int value = [number intValue];
//                    CCLOG(@"00 value :%d",value);
					contentValue = tempResult[value];
//                    CCLOG(@"00 contentValue :%d",contentValue);
...
//                CCLOG(@"01 contentValue : -1");
				tile = [[Tile alloc] initWithX: columnIndex y: rowIndex kind:-1];
			}else {
                CCLOG(@"03 spare :%d",spare);
                
                if (spare == 0) {
                    spare =1;
                }
				int index = arc4random()% spare;
                CCLOG(@"03 index :%d",index);
				NSNumber *number = [indexArray objectAtIndex: index];
				int value = [number intValue];
                CCLOG(@"03 value :%d",value);
				contentValue = tempResult[value];
                CCLOG(@"03 contentValue :%d",contentValue);
				[indexArray removeObjectAtIndex: index];
				tile = [[Tile alloc] initWithX: columnIndex y: rowIndex kind: contentValue];
				[ballTiles addObject: tile];
				spare--;
