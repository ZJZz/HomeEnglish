...
//        NSString *btnFrame = (big) ? @"button_big.png" : @"button_small.png";
        NSString *btnFrame = @"button_big.png";
        int fSize = 12;
        
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"jetpack-hd.plist"];
        
//        [self setDisplayFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:btnFrame]];
        [self setDisplayFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:btnFrame]];
        CCLabelTTF *label = [CCLabelTTF labelWithString:buttonText fontName:@"TOONISH" fontSize:fSize ];
        label.position = ccp(self.contentSize.width/2, self.contentSize.height/2);
        [self addChild:label z:1];
        CCLabelTTF *labelShadow = [CCLabelTTF labelWithString:buttonText fontName:@"TOONISH" fontSize:fSize];
        labelShadow.position = ccp(self.contentSize.width/2 - (2), self.contentSize.height/2);
