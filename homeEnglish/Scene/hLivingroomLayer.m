...
@implementation hLivingroomLayer
@synthesize wordsLivingroom;
...
+(CCScene *) scene
{
	...
	hLivingroomLayer *layer = [hLivingroomLayer node];
...
-(id) init
{
	...
	if( (self=[super init])) {
        ...
        wordsLivingroom = [[NSDictionary alloc] initWithObjectsAndKeys:
                           @"cat",@"252",
                           @"video-camera",@"249",
                           @"camera",@"246",
                           @"CD",@"243",
        ...
        //Animation frame
        moleFrameCountArray = [[NSArray alloc] initWithObjects:@"13", @"17", @"20",@"17",@"12",@"14",@"14",@"8",@"6",@"3", nil];
        //Animation frame name
        moleFirstNameArray = [[NSArray alloc] initWithObjects:
                              @"LivingroomAnimMagic",
                              @"LivingroomAnimDot",
                              @"LivingroomAnimColoriage",
                              @"LivingroomAnimStory",
                              @"LivingroomAnimOdd",
                              @"LivingroomAnimMemory",
                              @"LivingroomAnimDifference",
                              @"RemoteAnim",
                              @"MicroAnim",
                              @"homeReturnBt",  
                              nil];
        //Animation Position XY
        molePositonXArray = [[NSArray alloc] initWithObjects:@"452", @"0", @"432" ,@"240",@"663",@"539",@"800",@"569",@"161",@"954", nil];
        molePositonYArray = [[NSArray alloc] initWithObjects:@"187", @"0", @"0" ,@"354",@"60",@"446",@"397",@"576",@"91",@"698", nil];
        //Background sound
        [[SimpleAudioEngine sharedEngine] preloadBackgroundMusic:@"Livingroommainbg.mp3"];
        [CDAudioManager sharedManager].backgroundMusic.volume = 0.5f;
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"Livingroommainbg.mp3" loop:YES];
        
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Livingroomanimall.plist"];

        
        //Background
        CCSprite *bg = [CCSprite spriteWithFile:@"LivingroomMain.png"];
...
-(void)helpSceneNamePlay{
    ...
    else
    {
        //音效没有播放
        CCLOG(@"play");
        myEffect=[[[SimpleAudioEngine sharedEngine] soundSourceForFile:@"Livingroom_help.mp3"] retain];
        [myEffect play];
...
- (void) playWordSound:(CGPoint)location{
    //判断单词
    int clickColorTemp;
    NSString *imageName = @"LivingroomAlpha.png";
    ...
    if ( clickColorTemp != 255 ) {
        ...
        wordName = [wordsLivingroom objectForKey:keyString];
...
- (void) dealloc
{
	...
	[wordsLivingroom release];
    
    [[CCSpriteFrameCache sharedSpriteFrameCache] removeSpriteFramesFromFile:@"Livingroomanimall.plist"];
