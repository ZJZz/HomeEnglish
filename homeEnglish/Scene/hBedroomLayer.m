...
@implementation hBedroomLayer
@synthesize wordsBedroom;
...
+(CCScene *) scene
{
	...
	hBedroomLayer *layer = [hBedroomLayer node];
...
-(id) init
{
	...
	if( (self=[super init])) {
        ...
        wordsBedroom = [[NSDictionary alloc] initWithObjectsAndKeys:
                        @"painting",@"252",
                        @"shoes",@"249",
                        @"baseball_cap",@"246",
                        @"teddy_bear",@"243",
        ...
        //Animation frame
        moleFrameCountArray = [[NSArray alloc] initWithObjects:@"8", @"16", @"20" ,@"14",@"7",@"14",@"15",@"8",@"6",@"3", nil];
        //Animation frame name
        moleFirstNameArray = [[NSArray alloc] initWithObjects:
                              @"BedroomAnimMagic",
                              @"BedroomAnimDot",
                              @"BedroomAnimColoriage",
                              @"BedroomAnimStory",
                              @"BedroomAnimOdd",
                              @"BedroomAnimMemory",
                              @"BedroomAnimDifference",
                              @"RemoteAnim",
                              @"MicroAnim",
                              @"homeReturnBt",  
                              nil];
        //Animation Position XY
        molePositonXArray = [[NSArray alloc] initWithObjects:@"469", @"0", @"823" ,@"0",@"920",@"111",@"283",@"623",@"116",@"954", nil];
        molePositonYArray = [[NSArray alloc] initWithObjects:@"768", @"615", @"128" ,@"464",@"606",@"183",@"266",@"71",@"360",@"70", nil];
        
        //Background sound
        [[SimpleAudioEngine sharedEngine] preloadBackgroundMusic:@"Bedroommainbg.mp3"];
        [CDAudioManager sharedManager].backgroundMusic.volume = 0.5f;
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"Bedroommainbg.mp3" loop:YES];
        
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Bedroomanimall.plist"];

        //Background
        CCSprite *bg = [CCSprite spriteWithFile:@"BedroomMain.png"];
...
-(void)helpSceneNamePlay{
    ...
    else
    {
        //音效没有播放
        CCLOG(@"play");
        myEffect=[[[SimpleAudioEngine sharedEngine] soundSourceForFile:@"Bedroom_help.mp3"] retain];
        [myEffect play];
...
- (void) playWordSound:(CGPoint)location{
    //判断单词
    int clickColorTemp;
    NSString *imageName = @"BedroomAlpha.png";
    ...
    if ( clickColorTemp != 255 ) {
        ...
        wordName = [wordsBedroom objectForKey:keyString];
...
- (void) dealloc
{
	...
	[wordsBedroom release];
    
    [[CCSpriteFrameCache sharedSpriteFrameCache] removeSpriteFramesFromFile:@"Bedroomanimall.plist"];
