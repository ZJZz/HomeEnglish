...
@implementation hBathroomLayer
@synthesize wordsBathroom;
...
+(CCScene *) scene
{
	...
	hBathroomLayer *layer = [hBathroomLayer node];
...
//        self.scale = 2;
        ...
        wordsBathroom = [[NSDictionary alloc] initWithObjectsAndKeys:
                         @"brush",@"252",
                         @"comb",@"249",
                         @"mirror",@"246",
                         @"towel",@"243",
        ...
        //Animation frame
        moleFrameCountArray = [[NSArray alloc] initWithObjects:@"13", @"16", @"19",@"11",@"12",@"22",@"9",@"8",@"6",@"3", nil];
        //Animation frame name
        moleFirstNameArray = [[NSArray alloc] initWithObjects:
                              @"BathroomAnimMagic",
                              @"BathroomAnimDot",
                              @"BathroomAnimColoriage",
                              @"BathroomAnimStory",
                              @"BathroomAnimOdd",
                              @"BathroomAnimMemory",
                              @"BathroomAnimDifference",
                              @"RemoteAnim",
                              @"MicroAnim",
                              @"homeReturnBt",  
                              nil];
        //Animation Position XY
        molePositonXArray = [[NSArray alloc] initWithObjects:@"491", @"694", @"73" ,@"865",@"773",@"365",@"44",@"281",@"104",@"954", nil];
        molePositonYArray = [[NSArray alloc] initWithObjects:@"768", @"714", @"372" ,@"655",@"550",@"645",@"768",@"206",@"598",@"70", nil];
        
        //Background sound
        [[SimpleAudioEngine sharedEngine] preloadBackgroundMusic:@"Bathroommainbg.mp3"];
        [CDAudioManager sharedManager].backgroundMusic.volume = 0.5f;
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"Bathroommainbg.mp3" loop:YES];
        
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Bathroomanimall.plist"];
        //Background
        CCSprite *bg = [CCSprite spriteWithFile:@"BathroomMain.png"];
        ...
        CCLOG(@"fromAPPusername:%@",[SceneTo fromAPPusername]);
        ...
        //      1       2   3       4       5       6       7
        if ([[OldNameSave objectAtIndex:userCurentIndex+2] integerValue]) {
            helpSoundPlayIFONT = FALSE;
        }else{
            [OldNameSave replaceObjectAtIndex:userCurentIndex+2 withObject:[NSMutableString stringWithString:@"1"]];
...
-(void)helpSceneNamePlay{
    ...
    else
    {
        //音效没有播放
        CCLOG(@"play");
        myEffect=[[[SimpleAudioEngine sharedEngine] soundSourceForFile:@"Bathroom_help.mp3"] retain];
        [myEffect play];
...
- (void) playWordSound:(CGPoint)location{
    //判断单词
    int clickColorTemp;
    NSString *imageName = @"BathroomAlpha.png";
    ...
    if ( clickColorTemp != 255 ) {
        ...
        wordName = [wordsBathroom objectForKey:keyString];
...
- (void) dealloc
{
	...
	[wordsBathroom release];
    
    [[CCSpriteFrameCache sharedSpriteFrameCache] removeSpriteFramesFromFile:@"Bathroomanimall.plist"];
