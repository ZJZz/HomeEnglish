...
@implementation hGardenLayer
@synthesize wordsGarden;
...
+(CCScene *) scene
{
	...
	hGardenLayer *layer = [hGardenLayer node];
...
-(id) init
{
	...
	if( (self=[super init])) {
        ...
        wordsGarden = [[NSDictionary alloc] initWithObjectsAndKeys:
                       @"cloud",@"252",
                       @"tree",@"249",
                       @"cow",@"246",
                       @"orange_fruit",@"243",
        ...
        //Animation frame
        moleFrameCountArray = [[NSArray alloc] initWithObjects:@"13", @"9", @"10" ,@"17",@"11",@"14",@"9",@"8",@"6",@"3", nil];
        ...
        moleFirstNameArray = [[NSArray alloc] initWithObjects:
                              ...
                              @"GardenAnimOdd",
                              @"GardenAnimMemory",
                              @"GardenAnimDifference",
                              @"RemoteAnim",
                              @"MicroAnim",
                              @"homeReturnBt",  
                              nil];
        //Animation Position XY
        molePositonXArray = [[NSArray alloc] initWithObjects:@"603", @"0", @"759" ,@"46",@"347",@"491",@"202",@"578",@"210",@"954", nil];
        molePositonYArray = [[NSArray alloc] initWithObjects:@"17", @"33", @"159" ,@"253",@"138",@"629",@"589",@"296",@"142",@"698", nil];
        
        //Background sound
        [[SimpleAudioEngine sharedEngine] preloadBackgroundMusic:@"Gardenmainbg.mp3"];
        [CDAudioManager sharedManager].backgroundMusic.volume = 0.5f;
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"Gardenmainbg.mp3" loop:YES];
        
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Gardenanimall.plist"];

        
        
        //Background
        CCSprite *bg = [CCSprite spriteWithFile:@"GardenMain.png"];
...
-(void)helpSceneNamePlay{
    ...
    else
    {
        //音效没有播放
        CCLOG(@"play");
        myEffect=[[[SimpleAudioEngine sharedEngine] soundSourceForFile:@"Garden_help.mp3"] retain];
        [myEffect play];
...
//                case 2:
//                    //场景跳转，并且解决单词和热点区域重复问题。
//                    
//                    //播放9个热点动画
//                    for (int i=0; i<9; i++) {
//                        if (i!=2 && i!=3 && i!=4) {
//                        if (CGRectContainsPoint([mole[i] boundingBox], location)) {
//                            [SceneTo toSceneGame:i];                                  
//                        }
//                        }
//                    }
//                    break;
...
- (void) playWordSound:(CGPoint)location{
    //判断单词
    int clickColorTemp;
    NSString *imageName = @"GardenAlpha.png";
    ...
    if ( clickColorTemp != 255 ) {
        ...
        wordName = [wordsGarden objectForKey:keyString];
...
- (void) dealloc
{
    ...
    [[CCSpriteFrameCache sharedSpriteFrameCache] removeSpriteFramesFromFile:@"Gardenanimall.plist"];
