...
@implementation hLibraryLayer
@synthesize wordsLibrary;
...
+(CCScene *) scene
{
	...
	hLibraryLayer *layer = [hLibraryLayer node];
...
-(id) init
{
	...
	if( (self=[super init])) {
        ...
        wordsLibrary = [[NSDictionary alloc] initWithObjectsAndKeys:
                        @"chalkboard",@"252",
                        @"computer",@"249",
                        @"pen",@"246",
                        @"pencil",@"243",
        ...
        //Animation frame
        moleFrameCountArray = [[NSArray alloc] initWithObjects:@"10", @"16", @"13",@"16",@"7",@"14",@"13",@"8",@"6",@"3", nil];
        //Animation frame name
        moleFirstNameArray = [[NSArray alloc] initWithObjects:
                              @"LibraryAnimMagic",
                              @"LibraryAnimDot",
                              @"LibraryAnimColoriage",
                              @"LibraryAnimStory",
                              @"LibraryAnimOddOneOut",
                              @"LibraryAnimMemory",
                              @"LibraryAnimDifference",
                              @"RemoteAnim",
                              @"MicroAnim",
                              @"homeReturnBt",  
                              nil];
        //Animation Position XY
        molePositonXArray = [[NSArray alloc] initWithObjects:@"642", @"968", @"0" ,@"390",@"518",@"897",@"0",@"779",@"65",@"954", nil];
        molePositonYArray = [[NSArray alloc] initWithObjects:@"544", @"657", @"343" ,@"691",@"282",@"257",@"768",@"306",@"651",@"70", nil];
        
        //Background sound
        [[SimpleAudioEngine sharedEngine] preloadBackgroundMusic:@"Librarymainbg.mp3"];
        [CDAudioManager sharedManager].backgroundMusic.volume = 0.5f;
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"Librarymainbg.mp3" loop:YES];
        
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Libraryanimall.plist"];

        
        //Background
        CCSprite *bg = [CCSprite spriteWithFile:@"LibraryMain.png"];
...
-(void)helpSceneNamePlay{
    ...
    else
    {
        //音效没有播放
        CCLOG(@"play");
        myEffect=[[[SimpleAudioEngine sharedEngine] soundSourceForFile:@"Library_help.mp3"] retain];
        [myEffect play];
...
- (void) playWordSound:(CGPoint)location{
    //判断单词
    int clickColorTemp;
    NSString *imageName = @"LibraryAlpha.png";
    ...
    if ( clickColorTemp != 255 ) {
        ...
        wordName = [wordsLibrary objectForKey:keyString];
...
- (void) dealloc
{
	...
	[wordsLibrary release];
    
    [[CCSpriteFrameCache sharedSpriteFrameCache] removeSpriteFramesFromFile:@"Libraryanimall.plist"];
